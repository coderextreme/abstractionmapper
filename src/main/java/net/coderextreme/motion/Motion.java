...
*/
{
	...
	static String image_dir = "net/coderextreme/motion/gedda/";
	boolean selact = true;
	...
	public static void main (String argv[]) {
		...
		jmi.addActionListener(new OpenAction());
		file.add(jmi);
...
*/
		...
		if (typearea == null) {
			typearea = new MUDClient(/*root.id()*/);
	...
	class IMD extends JButton {
		public IMD(ImageIcon ii) {
			super(ii);
			net.coderextreme.motion.dnd.TransferActionListener tal = new net.coderextreme.motion.dnd.TransferActionListener();
...
// run SELECTED operations
	...
	void remove(Vector list)
	{
		...
		for (Enumeration e = list.elements();
			...
			JComponent jc = (JComponent)e.nextElement();
			MUDRemote o = (MUDRemote)jc.getClientProperty("object");
			Vector<BrowseRelationship>  removeRels = new Vector(2);
			for (e = relationships.elements(); e != null && e.hasMoreElements();) {
				BrowseRelationship br = (BrowseRelationship)e.nextElement();
